...
use crate::utils_rust::{geometry_utils};
...
pub struct CollisionNNConstructorData {
...
impl CollisionNN {
    pub fn new(config: CollisionNNConstructorData) -> Self {

        let input_length = config.coefs[0].len();
        let mut coef_matrices: Vec<DMatrix<f64>> = Vec::new();
        let mut intercept_vectors: Vec<DMatrix<f64>> = Vec::new();
        for i in 0..config.coefs.len() {
            let mut m = DMatrix::from_element( config.coefs[i].len(), 
                                                config.coefs[i][0].len(), 0.0 );
            for j in 0..config.coefs[i].len() {
                for k in 0..config.coefs[i][j].len() {
                    m[(j,k)] = config.coefs[i][j][k];
                }
            }
            coef_matrices.push(m);
        }

        for i in 0..config.intercepts.len() {
            let mut v = DMatrix::from_element(1, config.intercepts[i].len(),0.0);
            for j in 0..config.intercepts[i].len() {
                v[j] = config.intercepts[i][j];
            }
        ...
        for i in 0..intercept_vectors.len() {
            __intermediate_vecs.push(intercept_vectors[i].clone());
        }

        Self{coef_matrices: coef_matrices, 
            intercept_vectors: intercept_vectors.clone(), 
            split_point: config.split_point, 
            input_length, 
            result, __x_proxy, __intermediate_vecs}
    }
