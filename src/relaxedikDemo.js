...
export async function relaxedikDemo() {
    ...
    getURDFFromURL("../models/sawyer_description/urdf/sawyer_gripper.urdf", (blob) => {
        loadRobot(URL.createObjectURL(blob))
    });

    getURDFFromURL("../models/Kitchen_updated/Kitchen_dynamic/urdf/kitchen_dynamic.urdf", (blob) => {
        loadKitchenDynamic(URL.createObjectURL(blob))
    });

    // refridgerator, props (plates, microwave, bowls, etc.)
    getURDFFromURL("../models/Kitchen_updated/Kitchen_standard/urdf/Kitchen_standard.urdf", (blob) => {
        loadKitchenStandard(URL.createObjectURL(blob))
    });

    // kitchen 
    getURDFFromURL("../models/Kitchen_updated/Kitchen_static/urdf/Kitchen_static.urdf", (blob) => {
